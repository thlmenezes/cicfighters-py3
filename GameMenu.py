...
class GameMenu(object):
        ...
        if jogadores is None:
            jogadores = ["", "",
                [
                    ...
                    pygame.K_LEFT,
                    pygame.K_RIGHT,
                    # NOTE: KP means keypad, changed for testing in more keyboard layouts
    ...
    def inicializa(self,jogadores):
        ...
        self.fundo = Sprite.Sprite(os.path.join('.','Imagens','menu.png'),(0,0),0,(self.width,self.height))
    ...
    def update(self,dtempo,eventos):
        ...
        if self.configcontrole == 1:
            for event in eventos:
                if (event.type == pygame.KEYDOWN): self.botoesp1.append(event.key)
            ...
            for event in eventos:
                if (event.type == pygame.KEYDOWN): self.botoesp2.append(event.key)
